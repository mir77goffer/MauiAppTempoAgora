    ...
    public class DataService
    {
        public static async Task<Tempo?> GetPrevisao(string cidade)
        {
            ...
            Tempo tempo = new Tempo
            {
                ...
                description =
                    (string?)rascunho["weather"]?[0]?["description"],

                main =
                    (string?)rascunho["weather"]?[0]?["main"],

                temp_min =
                    (double?)rascunho["main"]?["temp_min"],

                temp_max =
                    (double?)rascunho["main"]?["temp_max"],

                speed =
                    (double?)rascunho["wind"]?["speed"],

                visibility =
                    (int?)rascunho["visibility"],
